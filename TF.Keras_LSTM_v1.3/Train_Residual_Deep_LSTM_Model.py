...
from Models_v01 import residual_deep_lstm_model
...
Window_Size = 16
Residual_Deep_LSTM_Model = residual_deep_lstm_model(Window_Size)
Residual_Deep_LSTM_Model = compileModel(Residual_Deep_LSTM_Model)
...
for i in range(Max_Epochs):
    ...
    History = fitModel(Residual_Deep_LSTM_Model, LSTM_Window, epochs=1)
    ...
    if i == 0:
        ...
        CheckPoint = os.path.join(checkpoint_filepath,
                                  'cp_5x10_' + str(int(len(Data) / 1000)) + 'k_samples_Random_Data_w_' + str(
                                      Window_Size) + '_' + str(
                                      i + 1) + '_epochs_Residual_Deep_LSTM_Model')
        Residual_Deep_LSTM_Model.save(CheckPoint)
        print(
            'checkpoint ' + '5x10 ' + str(int(len(Data) / 1000)) + 'k_samples_Random_Data_w_' + str(
                Window_Size) + ' ' + str(
                i + 1) + ' epochs _Residual_Deep_LSTM_Model' + ' is saved')
...
plt.grid()
plt.savefig(
    './Result_Plots/' + 'Residual_Deep_LSTM_Model_1k_samples_Random_Data_w_' + str(Window_Size) + '_' + str(
        i + 1) + '_epochs.png',
    bbox_inches='tight')
plt.show()
