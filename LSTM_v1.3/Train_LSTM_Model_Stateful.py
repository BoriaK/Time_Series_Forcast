...
from Models_v01 import lstm_model_stateful
...
LSTM_Model_Stateful = lstm_model_stateful(Window_Size, LSTM_Window.example[0].shape)
LSTM_Model_Stateful = compileModel(LSTM_Model_Stateful)
...
for i in range(Max_Epochs):
    ...
    History = fitModel(LSTM_Model_Stateful, LSTM_Window, epochs=1)
    ...
    if i == 0:
        ...
        CheckPoint = os.path.join(checkpoint_filepath, 'cp_1x10_' + '10k samples ' + str(i + 1) + '_epochs_LSTM_Model_Stateful')
        LSTM_Model_Stateful.save(CheckPoint)
        print('checkpoint ' + '1x10_' + '10k samples ' + str(i + 1) + '_epochs_LSTM_Model_Stateful' + ' is saved')
...
plt.grid()
plt.savefig('./Result_Plots/' + 'LSTM_Model_Stateful ' + '10k samples ' + str(i + 1) + '_epochs.png', bbox_inches='tight')
plt.show()
