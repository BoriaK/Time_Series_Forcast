...
from Models_v01 import deep_conv_model
...
Conv_Width = 16
Deep_Conv_Model = deep_conv_model(Conv_Width)
Deep_Conv_Model = compileModel(Deep_Conv_Model)
...
for i in range(Max_Epochs):
    ...
    History = fitModel(Deep_Conv_Model, Conv_Window, epochs=1)
    ...
    if i == 0:
        ...
        CheckPoint = os.path.join(checkpoint_filepath,
                                  'cp_2x16_' + str(int(len(Data) / 1000)) + 'k_samples_Random_Data_w_' + str(
                                      Conv_Width) + '_' + str(
                                      i + 1) + '_epochs_Deep_Conv_Model')
        Deep_Conv_Model.save(CheckPoint)
        print(
            'checkpoint ' + '2x16 ' + str(int(len(Data) / 1000)) + 'k_samples_Random_Data_w_' + str(Conv_Width) + ' ' + str(
                i + 1) + ' epochs Deep_Conv_Model' + ' is saved')
...
plt.grid()
plt.savefig(
    './Result_Plots/Training_Process/' + '2x16Deep_Conv_Model_1k_samples_Random_Data_w_' + str(Conv_Width) + '_' + str(
        i + 1) + '_epochs.png',
    bbox_inches='tight')
plt.show()
