...
from Models_v01 import cnn_lstm
...
Window_Size = 16
CNN_LSTM_Model = cnn_lstm(Window_Size)
    ...
    if i == 0:
        ...
        CheckPoint = os.path.join(checkpoint_filepath,
                                  'cp_1x16_' + str(int(len(Data) / 1000)) + 'k_samples_Random_Data_w_' + str(
                                      Window_Size) + '_' + str(
                                      i + 1) + '_epochs_CNN_LSTM_Model')
        CNN_LSTM_Model.save(CheckPoint)
        print(
            'checkpoint ' + '1x16 ' + str(int(len(Data) / 1000)) + 'k_samples_Random_Data_w_' + str(Window_Size) + ' ' + str(
...
plt.grid()
plt.savefig(
    './Result_Plots/' + 'CNN_LSTM_Model_1k_samples_Random_Data_w_' + str(Window_Size) + '_' + str(
        i + 1) + '_epochs.png',
    bbox_inches='tight')
plt.show()
